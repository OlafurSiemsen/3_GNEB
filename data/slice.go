...
/*
Takes in slice as generated by variadic parameter, returns first
member if the variadic parameter has non-zero size, otherwise 1.
Panics if the parameter is malformed.
*/
func ImageCheck(n_images []int) int {
	switch len(n_images) {
	case 0:
		return 1
	case 1:
...
func NewSlice(nComp int, size [3]int, n_images_variadic ...int) *Slice {
	var length int
	n_images := ImageCheck(n_images_variadic)
	if n_images != 1 {
...
func NilSlice(nComp int, size [3]int, n_images_slice ...int) *Slice {
	n_images := ImageCheck(n_images_slice)
	if n_images != 1 {
...
func Index(size [3]int, ix, iy, iz int, image_param ...int) int {
	...
	if len(image_param) == 2 {
		n_images = image_param[0]
		i_image = image_param[1]
	} else {
