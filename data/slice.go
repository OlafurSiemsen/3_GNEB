...
type Slice struct {
	...
	// set to 0 for an empty slice, 1 for a slice with non-GNEB usage
	n_images int
}
...
// Takes in slice as generated by variadic parameter, returns true if
// len == 1, false if len == 0, panics otherwise
func GNEBCheck(n_images []int) bool {
	switch len(n_images) {
	case 0:
		return false
	case 1:
...
func NewSlice(nComp int, size [3]int, n_images ...int) *Slice {
	var length int
	t_GNEBCheck := GNEBCheck(n_images)
	if t_GNEBCheck {
		length = n_images[0] * prod(size)
	} else {
		length = prod(size)
	}
	...
	if t_GNEBCheck {
		return SliceFromPtrs(size, CPUMemory, ptrs, n_images[0])
	} else {
		return SliceFromPtrs(size, CPUMemory, ptrs)
	}
}

func SliceFromArray(data [][]float32, size [3]int, n_images ...int) *Slice {
	nComp := len(data)
	var length int
	t_GNEBCheck := GNEBCheck(n_images)
	if t_GNEBCheck {
...
func NilSlice(nComp int, size [3]int, n_images ...int) *Slice {
	t_GNEBCheck := GNEBCheck(n_images)
	if t_GNEBCheck {
		return SliceFromPtrs(size, GPUMemory, make([]unsafe.Pointer, nComp, n_images[0]))
	} else {
...
func SliceFromPtrs(size [3]int, memType int8, ptrs []unsafe.Pointer, n_images ...int) *Slice {
	var length int
	t_GNEBCheck := GNEBCheck(n_images)
	if t_GNEBCheck { // Checks if we're doing GNEB
		length = n_images[0] * prod(size)
	} else {
		length = prod(size)
	}
