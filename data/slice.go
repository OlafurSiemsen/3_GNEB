...
type Slice struct {
	...
	size    [3]int
	memType int8
	// Keeps track of the number of NEB images
...
/*
Takes in slice as generated by variadic parameter, and extracts image number.
*/
func ImageNumber(n_images []int) int {
	switch len(n_images) {
	case 0:
		return 1
	case 1:
		return n_images[0]
	default:
		panic("Unexpected number of variadic parameters")
	}
}

// TODO-olafur: Move to utils?
// Takes in slice as generated by variadic parameter, and extracts image index.
func ImageIndex(i_image []int) int {
	switch len(i_image) {
	case 0:
		return 0
	case 1:
...
// TODO-olafur: Move to utils?
// Takes in slice as generated by variadic parameter, and extracts image number and image index.
func ImageParam(param_image []int) [2]int {
	switch len(param_image) {
	case 0:
		return [2]int{1, 0}
	case 2:
		return [2]int{param_image[0], param_image[1]}
	default:
		panic("Unexpected number of variadic parameters")
	}
}
...
func NewSlice(nComp int, size [3]int, n_images_variadic ...int) *Slice {
	var length int
	...
	if n_images != 1 {
		return SliceFromPtrs(size, CPUMemory, ptrs, n_images)
	} else {
		return SliceFromPtrs(size, CPUMemory, ptrs)
	}
}

func SliceFromArray(data [][]float32, size [3]int, n_images_variadic ...int) *Slice {
	nComp := len(data)
	var length int
	n_images := ImageNumber(n_images_variadic)
	if n_images != 1 {
...
// Note that it will return all the GNEB images concatenated
...
func Index(size [3]int, ix, iy, iz int, param_image_variadic ...int) int {
	t_params := ImageParam(param_image_variadic)
	n_images := t_params[0]
	ind_image := t_params[1]
	if ix < 0 || ix >= size[X] || iy < 0 || iy >= size[Y] || iz < 0 || iz >= size[Z] || ind_image < 0 || ind_image >= n_images {
		panic(fmt.Sprintf("Slice index out of bounds: %v,%v,%v (bounds=%v)\n", ix, iy, iz, size))
	}
	return ((ind_image*size[Z]+iz)*size[Y]+iy)*size[X] + ix
...
func (i_slice *Slice) SubSlice(ind_image int) *Slice {
	...
	for i := range NComp {
		ptrs[i] = unsafe.Pointer(uintptr(i_slice.ptrs[i]) + uintptr(ind_image*image_offset))
	}
