...
/*
Takes in slice as generated by variadic parameter, and extract image number.
*/
...
/*
Takes in slice as generated by variadic parameter, and extract image index.
*/
...
func (s *Slice) Index(ix, iy, iz int, i_image_variadic ...int) int {
	var i_image int
	if len(i_image_variadic) == 1 {
		i_image = i_image_variadic[0]
	} else {
		i_image = 0
	}
...
func Index(size [3]int, ix, iy, iz int, image_param_variadic ...int) int {
	var i_image, n_images int
	if len(image_param_variadic) == 2 {
		n_images = image_param_variadic[0]
		i_image = image_param_variadic[1]
	} else {
		n_images, i_image = 1, 0
	}
	if ix < 0 || ix >= size[X] || iy < 0 || iy >= size[Y] || iz < 0 || iz >= size[Z] || i_image < 0 || i_image >= n_images {
