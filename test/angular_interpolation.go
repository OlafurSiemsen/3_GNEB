...
func main() {
	...
	// Checking that angles (dot products) are the same between adjacent images
	...
	for t_ind := range end_slice_ind - start_slice_ind - 1 {
		CompareSlices(t_dotprod_slice.SubSlice(t_ind), t_dotprod_slice.SubSlice(t_ind+1), false)
	}
}
