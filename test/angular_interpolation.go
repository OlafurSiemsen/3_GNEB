...
func main() {
	...
	fmt.Println("Testing the the generic case")
	n_images := 6
	...
	fmt.Println("Testing the case of anti-parallel images")
	n_images = 8
	start_slice_ind := 1
	end_slice_ind := 6
	antiparallel_test_slice := cuda.NewSlice(3, gridsize, n_images)
	t_slice := data.NewSlice(3, gridsize, 8)
	rng := rand.New(rand.NewSource(int64(777)))
	// rng := rand.New(rand.NewSource(int64(time.Now().UTC().UnixNano()))) RESTORE LATER!
