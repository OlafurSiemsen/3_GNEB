...
func SetDemagField(dst *data.Slice, mag *magnetization) {
	if EnableDemag {
		...
		if NoDemagSpins.isZero() {
			...
			demagConv().Exec(dst, mag.Buffer(), Universe_geometry.Gpu(), msat)
		} else {
...
func setMaskedDemagField(dst *data.Slice, msat cuda.MSlice) {
	...
	buf := cuda.Buffer(SCALAR, Universe_geometry.Gpu().Size()) // masked-out geometry
	defer cuda.Recycle(buf)

	// obtain a copy of the geometry mask, which we can overwrite
	geom, r := Universe_geometry.Slice()
	if r {
	...
	// mask-out
	cuda.ZeroMask(buf, NoDemagSpins.gpuLUT1(), Universe_regions.Gpu())
	...
	cuda.ZeroMask(dst, NoDemagSpins.gpuLUT1(), Universe_regions.Gpu())
}
...
func SetMFull(dst *data.Slice) {
	...
	vol, rV := Universe_geometry.Slice()
	if rV {
